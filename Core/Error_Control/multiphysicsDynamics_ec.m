function [Interactions,Calibration,excludePhysics,displayProgress] ...
    = multiphysicsDynamics_ec(Config,varargin_cell)
        ...
        case 'calibration' % calibration parameters
            ...
            if ~isValid
                warning(['Non-valid value for PROPERTY = ''Calibration'' '...
                    ...
                    'the ''Calibration'' structure: TAUG = %0.3e, '...
                    'TAUI = %0.3e, BETA = %0.3e, ETA = %0.3e, '...
                    'EPSIL = %0.3e, KVIS = %0.3e'],Calibration.taug,...
                    Calibration.taui,Calibration.beta,Calibration.eta,...
                    Calibration.epsil,Calibration.kvis)
            end
                        
        case 'excludephysics' % effective hydrostatic pressure
            options = values{m};
            validOptions = {'jt','mv','qw','qv','vg','mu','zb'};
            if ~isempty(options)
                if ischar(options)
